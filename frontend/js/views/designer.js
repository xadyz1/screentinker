...
export async function render(container) {
  ...
  if (hash.startsWith('#/designer/')) {
    const id = hash.split('#/designer/')[1];
...
async function renderList(container) {
  ...
  document.getElementById('newLayoutBtn').onclick = async () => {
    ...
    window.location.hash = `#/designer/${layout.id}`;
  };
  ...
  try {
    ...
    container.querySelectorAll('[data-edit-layout]').forEach(btn => {
      btn.onclick = () => { window.location.hash = `#/designer/${btn.dataset.editLayout}`; };
    });
...
async function renderEditor(container, layoutId) {
  ...
  container.innerHTML = `
    <a href="#/designer" class="back-link" style="display:inline-flex;align-items:center;gap:6px;color:var(--text-secondary);margin-bottom:16px;font-size:13px">
